...
export default function CardItem({ card, onClick, deckCount, collectionCount }: Props) {
  ...
  const collectionBg = deckCount && collectionCount !== undefined && collectionCount < deckCount
    ? "var(--color-warn)" : "var(--color-good)";

  return (
    <div
      className="tcg"
      style={{ "--card-color": hex, "--card-glow": glow } as React.CSSProperties}
      onClick={onClick}
    >
      {card.card_image ? (
        <img className="art-img" src={card.card_image} alt={card.card_name} loading="lazy" />
      ) : (
        <div className="art">
      ...
      {card.card_cost !== null && card.card_cost !== undefined && (
        <div className="cost-badge">{card.card_cost}</div>
      )}

      {card.card_power !== null && card.card_power !== undefined && (
        <div className="power-badge">{card.card_power}</div>
      )}
      ...
      <div className="card-footer">
        ...
        <div className="card-meta">
          {card.card_type}
          {card.card_cost !== null && card.card_cost !== undefined && ` · ${card.card_cost}`}
          {card.card_power !== null && card.card_power !== undefined && ` · ${card.card_power}`}
        </div>
      </div>
    </div>
  );
